...
function registerSocketIO() {
    ...
    socket.on("eyepos", function (msg, cb) {
        ...
        // LGazePosImg: [],
        // RGazePosImg: [],
        currentConfig["LGazePos"] = [msg[0], msg[1]];
        currentConfig["RGazePos"] = [msg[2], msg[3]];
        ...
        let leftEye = new OpenSeadragon.Point(
            Math.round(msg[0] * screenWidth) - marginLeft,
            Math.round(msg[1] * screenHeight) - marginTop
        );
        let rightEye = new OpenSeadragon.Point(
            Math.round(msg[2] * screenWidth) - marginLeft,
            Math.round(msg[3] * screenHeight) - marginTop
        );
        let LGazePosImg = imageCoordinatesFromPixel(leftEye);
        let RGazePosImg = imageCoordinatesFromPixel(rightEye);
        currentConfig["LGazePosImg"] = LGazePosImg;
        currentConfig["RGazePosImg"] = RGazePosImg;

        currentEyeTrackerData["LGazePos"] = [msg[0], msg[1]];
        currentEyeTrackerData["RGazePos"] = [msg[2], msg[3]];
        currentEyeTrackerData["LGazePosImg"] = LGazePosImg;
        currentEyeTrackerData["RGazePosImg"] = RGazePosImg;
...
var updateFrame = function () {
    ...
    currentConfig["boundY"] = OSDviewer.viewport
        .getBoundsNoRotate()
        .getTopLeft().y;
    currentConfig["boundW"] = OSDviewer.viewport.getBoundsNoRotate().width;
    currentConfig["boundH"] = OSDviewer.viewport.getBoundsNoRotate().height;
};
...
function getOSDViewerParams(config) {
    ...
    config["boundH"] = OSDviewer.viewport.getBoundsNoRotate().height;
    ...
    //console.log(zoom);
    config["zoom"] = zoom;
    config["imageZoom"] = OSDviewer.viewport.viewportToImageZoom(zoom);
    config["FPS"] = FPS;
...
function registerOSDTrackerEvents() {
    OSDviewer.addHandler("open", function () {
        ...
        $(".navigator").mousemove(function (event) {
            ...
            currentConfig["mousePos"] = [event.pageX, event.pageY];
            trackerData.push(currentConfig);
            // console.log("move in navigator", currentConfig);
        });
        ...
        anno.on("changeSelectionTarget", function (target) {
            ...
            currentConfig["mousePosImg"] = imageCoordinatesFromPixel(position);
            trackerData.push(currentConfig);
            // console.log("start editting annotation");
        });
    });
}
...
function registerAnnotoriousEvents() {
    ...
    anno.on("updateAnnotation", async function (annotation, previous) {
        ...
        //currentConfig["mousePos"] = [event.pageX, event.pageY];
        //let position = new OpenSeadragon.Point(event.offsetX, event.offsetY);
        //currentConfig["mousePosImg"] = imageCoordinatesFromPixel(position);
        trackerData.push(currentConfig);

        let currentAnnoRes = JSON.stringify(anno.getAnnotations());
        await updateLabel(username, image_id, "regions", currentAnnoRes, "str");
        console.log("finish_editing_region");
    });
...
async function saveTextLabels() {
    ...
    if (validateText) {
        $(".text-label").each(async function () {
            ...
            currentConfig[
                "eventLog"
            ] = `update_text: set ${label_id} to ${text}`;
            trackerData.push(currentConfig);
            //console.log("UI: update text", currentConfig);
        });
    } else {
