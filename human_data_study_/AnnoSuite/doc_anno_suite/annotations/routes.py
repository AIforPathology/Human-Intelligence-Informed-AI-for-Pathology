...
def annotate_study():
    ...
    if(is_ready):
        schemas = DatabaseCRUD.get_label_schemas([-1])
        # print(schemas)
        return render_template('annotate-study.html', title='Annotate', schemas=schemas, username=str(current_user.username), url_image_id=url_image_id)
    else:
        if(current_user.is_admin):
            return redirect(url_for('management.admin'))
        else:
...
# only used for getting the navigator size
@annotations.route("/update_image_captionurl", methods=['POST'])
def update_image_captionurl():
    if request.method == 'POST':
        image_id = request.form.get('image_id')
        navigator_meta = request.form.get('navigator_meta')
        DatabaseCRUD.update_image_captionurl(image_id, navigator_meta)
        return {'status': 1}
